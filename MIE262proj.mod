...
- sum{s in S, m in M} (machine_op_cost[m]+(WORKER_WAGE/60))*x[s]*machine_op_time[s, m]
- sum{s in S} DEMAND_PENALTY*(shoe_demand[s]-x[s]);
...
subject to Budget_C: (sum{s in S, r in R} raw_cost[r]*x[s]*shoe_materials[s, r]) + (sum{s in S, m in M}(machine_op_cost[m]+WORKER_WAGE/60)*x[s]*machine_op_time[s, m]) <= BUDGET;