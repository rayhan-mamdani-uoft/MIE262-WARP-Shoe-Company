...
#Decision Variables
var x{S} >= 0; #num of each shoe produced

#Objective Function
maximize profit: sum{s in S} (shoe_price[s]*x[s]
- (sum{r in R} raw_cost[r]*x[s]*shoe_materials[s, r])
- (sum{m in M} (machine_op_cost[m]+WORKER_WAGE/60)*x[s]*machine_op_time[s, m])
- 10*(shoe_demand[s]-x[s]));


#Constraints
subject to Op_Time_C{m in M}: sum{s in S} x[s]*machine_op_time[s, m] <= MACHINE_TIME; #operating time of each machine cannot exceed the operation minutes in february
...
subject to Budget_C: sum{s in S} ((sum{r in R} raw_cost[r]*x[s]*shoe_materials[s, r]) + (sum{m in M}(machine_op_cost[m]+WORKER_WAGE/60)*x[s]*machine_op_time[s, m])) <= BUDGET;